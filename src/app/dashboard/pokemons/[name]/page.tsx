...
export const generateStaticParams = async () => {
  const pokemonNames = (await getPokemons(151)).map((pokemon) => ({ name: pokemon.name }));
...
export const generateMetadata = async ({ params }: Props): Promise<Metadata> => {
  try {
    ...
    return {
      title: `#${pokemon.id} ${pokemon.name} - Pokedex`,
      description: `Información del pokemon ${pokemon.name}`,
    };
  } catch (error) {
    return {
